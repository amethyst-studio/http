...
# Install the needed system dependencies.
RUN apt update && apt install -y apt-utils aspell-de aspell-en curl firebird-dev libc-client-dev libcurl3-dev libcurl4 libfreetype6-dev libicu-dev libjpeg62-turbo-dev libkrb5-dev libldap2-dev libldb-dev libmcrypt-dev libonig-dev libpng-dev libpq-dev libpspell-dev librecode-dev librecode0 libsnmp-dev libsqlite3-0 libsqlite3-dev libssl-dev libtidy-dev libxml2-dev libxml2-dev libxslt-dev libzip-dev mariadb-client postgresql-client
RUN apt autoremove -y && apt clean autoclean
